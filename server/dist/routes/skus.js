...
router.get('/search', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const matches = allSKUs.filter(sku => sku.keywords.some(keyword => keyword.includes(searchTerm)) ||
...
router.get('/:sku/availability', auth_1.authenticateToken, async (req, res) => {
    try {
